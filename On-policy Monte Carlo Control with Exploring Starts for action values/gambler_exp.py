...
if __name__ == "__main__":
    ...
    j = 0
    np.save("Value_Function", average_v_over_runs)
    filename = "ValueFunction"
    #save the correspondent arrays of the average of v over the runs in a .txt file called ValueFunction
    with open(filename, "w") as data_file:
        while j < len(average_v_over_runs):
            data_file.write("{0}\n".format(average_v_over_runs[j]))
            j = j + 1
